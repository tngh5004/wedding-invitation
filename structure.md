...
- **글씨체**: 나눔손글씨 느릿느릿체(네이버 클로바 무료 배포)를 woff2로 변환해 자체 호스팅 (`public/assets/fonts/`). 기기 설치 폰트와 무관하게 안드로이드/iOS 공통 적용.
- **커버 문구 이미지화**: 웹폰트(2.5MB) 다운로드 전에도 첫 화면이 손글씨체로 보이도록, 커버의 이름/일시/장소 문구를 폰트 글리프에서 추출한 벡터 SVG(`cover-text.svg`)로 표시. 하트는 핫핑크.
